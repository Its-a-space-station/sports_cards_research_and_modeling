...
- Produces (used by Task 5):
  ...
  - `classify_adjustment(windows, early_hi=3.5, late_lo=7.0, no_move=0.02, late_move=0.05, fast_ratio=0.8) -> pd.DataFrame` — per `event_id`: `pre_med` (t < 0, full pre-window — [−21, 0) under the calibrated ±21 d collection window; prose corrected 2026-09-17 to match the code, which always used `t_days < 0`), `early_med` (t ∈ [0, 3.5)), `late_med` (t ≥ 7, uncapped — the mature-adjustment window; prose corrected 2026-09-17 to match the code, which always used `t_days >= late_lo`), `cls` ∈ `{"fast", "intermediate", "late", "no_adjustment", "insufficient"}` (each segment needs ≥ 2 sales else `insufficient`). `late − pre ≤ no_move` → `no_adjustment`; `(early−pre)/(late−pre) ≥ fast_ratio` → `fast`; `|early−pre| ≤ no_move and late−pre ≥ late_move` → `late`; else `intermediate`.
